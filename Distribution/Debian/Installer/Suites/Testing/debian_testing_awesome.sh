...
if [ "$first" != 1 ];then
	if [ ! -f $tarball ]; then
		...
		*)
		...
		wget "https://github.com/MobilinuxApp/Mobiconsole-CLI/blob/master/Distribution/Debian/Rootfs/testing/${archurl}/debian-rootfs-${archurl}.tar.xz?raw=true" -O $tarball
	fi
	cur=`pwd`
	mkdir -p "$folder"
	cd "$folder"
...
wget --tries=20 $dlink/Installer/WindowManagers/Awesome/debian_awesome_wm.sh -O $folder/root/debian_awesome_wm.sh
clear
echo "Setting up the installation of AWESOME WM VNC"
...
echo "Done! "
echo " "
echo "Updating Components, have paitance!"
sed -i 's/buster/bullseye/g; s/10//g' /etc/os-release
sed -i 's/buster/bullseye/g; s/10//g' /usr/lib/os-release
echo " "
echo "Done!"
echo " "
...
clear
if [ ! -f /root/debian_awesome_wm.sh ]; then
    wget --tries=20 $dlink/Installer/WindowManagers/Awesome/debian_awesome_wm.sh -O /root/debian_awesome_wm.sh
    bash ~/debian_awesome_wm.sh
else
    bash ~/debian_awesome_wm.sh
fi
clear
if [ ! -f /usr/local/bin/vncserver-start ]; then
    wget --tries=20  $dlink/Installer/WindowManagers/Awesome/vncserver-start -O /usr/local/bin/vncserver-start
    wget --tries=20 $dlink/Installer/WindowManagers/Awesome/vncserver-stop -O /usr/local/bin/vncserver-stop
...
echo ' Welcome to Mobilinux | Debian Testing (bullseye)'
rm -rf /root/adduser.sh
rm -rf /root/debian_awesome_wm.sh
