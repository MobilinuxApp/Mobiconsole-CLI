...
if [ "$first" != 1 ];then
	if [ ! -f $tarball ]; then
		...
		*)
		...
		wget "https://github.com/MobilinuxApp/Mobiconsole-CLI/blob/master/Distribution/Debian/Rootfs/oldstable/${archurl}/debian-rootfs-${archurl}.tar.xz?raw=true" -O $tarball
	fi
	cur=`pwd`
	mkdir -p "$folder"
	cd "$folder"
