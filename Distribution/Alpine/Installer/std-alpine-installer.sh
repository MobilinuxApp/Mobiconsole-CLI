...
if [ "$check" -eq "1" ] > /dev/null 2>&1; then
	...
	*)
	...
	url=https://github.com/MobilinuxApp/Mobiconsole-CLI/blob/master/Distribution/Alpine/Rootfs/${archurl}/alpine-minirootfs-3.10.3-${archurl}.tar.gz?raw=true
...
if [ -d $folder/var ];then
	...
	rm -rf $folder/root/.bash_profile
  	echo "#!/bin/bash
              wget https://raw.githubusercontent.com/MobilinuxApp/Mobiconsole-CLI/master/Distribution/Alpine/Installer/DEs/XFCE4/alpine_xfce4_de.sh -O /root/alpine_xfce4_de.sh
              bash /root/alpine_xfce4_de.sh
              rm -rf /root/alpine_xfce4_de.sh
              clear
...
echo ' Welcome to Mobilinux | Alpine Linux '
