...
wget --tries=20 $dlink/Installer/DEs/LXDE/fedora_lxde_de.sh -O $folder/root/fedora_lxde_de.sh
clear
echo "Setting up the installation of LXDE VNC"
echo "#!/bin/bash
yum install wget -y
clear
if [ ! -f /root/fedora_lxde_de.sh ]; then
    wget --tries=20 $dlink/Installer/DEs/LXDE/fedora_lxde_de.sh -O /root/fedora_lxde_de.sh
    bash ~/fedora_lxde_de.sh
else
    bash ~/fedora_lxde_de.sh
fi
clear
