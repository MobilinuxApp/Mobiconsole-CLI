...
wget --tries=20 $dlink/Installer/WindowManager/Awesome/fedora_awesome_wm.sh -O $folder/root/fedora_awesome_wm.sh
clear
echo "Setting up the installation of AWESOME WM VNC"
echo "#!/bin/bash
yum install wget -y
clear
if [ ! -f /root/fedora_awesome_wm.sh ]; then
    wget --tries=20 $dlink/Installer/WindowManager/Awesome/fedora_awesome_wm.sh -O /root/fedora_awesome_wm.sh
    bash ~/fedora_awesome_wm.sh
else
    bash ~/fedora_awesome_wm.sh
fi
clear
