...
wget --tries=20 $dlink/Installer/DEs/LXQT/fedora_lxqt_de.sh -O $folder/root/fedora_lxqt_de.sh
clear
echo "Setting up the installation of LXQT VNC"
echo "#!/bin/bash
yum install wget sudo dialog -y
clear
if [ ! -f /root/fedora_lxqt_de.sh ]; then
    wget --tries=20 $dlink/Installer/DEs/LXQT/fedora_lxqt_de.sh -O /root/fedora_lxqt_de.sh
    bash ~/fedora_lxqt_de.sh
else
    bash ~/fedora_lxqt_de.sh
fi
clear
