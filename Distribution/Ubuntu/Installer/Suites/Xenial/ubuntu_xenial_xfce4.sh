...
wget --tries=20 $dlink/Installer/DEs/XFCE4/CompatabilityScripts/ubuntu-compact-xfce4.sh -O $folder/root/ubuntu-compact-xfce4.sh
clear
echo "Setting up the installation of XFCE4 VNC"

echo "APT::Acquire::Retries \"3\";" > $folder/etc/apt/apt.conf.d/80-retries #Setting APT retry count
echo "#!/bin/bash
rm -rf /etc/resolv.conf
echo 'nameserver 8.8.8.8' >> /etc/resolv.conf
apt update -y && apt install wget sudo dialog -y > /dev/null
if [ ! -f /root/ubuntu-compact-xfce4.sh ]; then
    wget --tries=20 $dlink/Installer/DEs/XFCE4/CompatabilityScripts/ubuntu-compact-xfce4.sh -O /root/ubuntu-compact-xfce4.sh
    bash ~/ubuntu-compact-xfce4.sh
else
	bash ~/ubuntu-compact-xfce4.sh
fi
clear
...
rm -rf /root/ubuntu-compact-xfce4.sh
