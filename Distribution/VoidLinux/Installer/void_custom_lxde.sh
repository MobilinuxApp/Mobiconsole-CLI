...
wget --tries=20 $dlink/Installer/DEs/LXDE/void_lxde_de.sh -O $folder/root/void_lxde_de.sh > /dev/null
clear
echo "Setting up the installation of LXDE VNC"

echo "#!/bin/bash
xbps-install -Su -y && xbps-install -S lxde xorg tigervnc wget sudo -y 
if [ ! -f /root/void_xfce4_de.sh ]; then
    wget --tries=20 $dlink/Installer/DEs/LXDE/void_lxde_de.sh -O /root/void_lxde_de.sh > /dev/null
    bash ~/void_lxde_de.sh
else
    bash ~/void_lxde_de.sh
fi
clear
if [ ! -f /usr/local/bin/vncserver-start ]; then
    wget --tries=20  $dlink/Installer/DEs/LXDE/vncserver-start -O /usr/local/bin/vncserver-start > /dev/null
    wget --tries=20 $dlink/Installer/DEs/LXDE/vncserver-stop -O /usr/local/bin/vncserver-stop > /dev/null
    chmod +x /usr/local/bin/vncserver-stop
    chmod +x /usr/local/bin/vncserver-start
fi
if [ ! -f /usr/bin/vncserver ]; then
    xbps-install -S lxde xorg tigervnc wget -y  > /dev/null
fi
