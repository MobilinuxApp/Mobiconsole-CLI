...
wget $dlink/Installer/DEs/MATE/arch_mate_de.sh -O $folder/root/arch_mate_de.sh
echo " #!/bin/bash
bash ~/additional.sh
pacman -Syyuu --noconfirm && pacman -S wget sudo --noconfirm 
mkdir -p ~/.vnc
clear
if [ ! -f /root/arch_mate_de.sh ]; then
    wget --tries=20 $dlink/Installer/DEs/MATE/arch_mate_de.sh -O /root/arch_mate_de.sh
    bash ~/arch_mate_de.sh
else
    bash ~/arch_mate_de.sh
fi
clear
