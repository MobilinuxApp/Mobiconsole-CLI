...
wget $dlink/Installer/DEs/XFCE4/arch_xfce4_de.sh -O $folder/root/arch_xfce4_de.sh
echo " #!/bin/bash
bash ~/additional.sh
pacman -Syyuu --noconfirm && pacman -S wget sudo --noconfirm 
mkdir -p ~/.vnc
clear
if [ ! -f /root/arch_xfce4_de.sh ]; then
    wget --tries=20 $dlink/Installer/DEs/XFCE4/arch_xfce4_de.sh -O /root/arch_xfce4_de.sh
    bash ~/arch_xfce4_de.sh
else
    bash ~/arch_xfce4_de.sh
fi
clear
