...
wget $dlink/Installer/DEs/LXDE/arch_lxde_de.sh -O $folder/root/arch_lxde_de.sh
echo " #!/bin/bash
bash ~/additional.sh
pacman -Syyuu --noconfirm && pacman -S wget sudo --noconfirm 
mkdir -p ~/.vnc
clear
if [ ! -f /root/arch_lxde_de.sh ]; then
    wget --tries=20 $dlink/Installer/DEs/LXDE/arch_lxde_de.sh -O /root/arch_lxde_de.sh
    bash ~/arch_lxde_de.sh
else
    bash ~/arch_lxde_de.sh
fi
clear
